...
class params:
    ...
    def single_graph(self, T, xCr, xAl, N, K):
        ...
        self.MCrCr = xCr*(((1.0 - xCr)*(1.0 - xCr))*self.MCr + xCr*xAl*self.MAl + xCr*self.xFe*self.MFe)
        self.MAlAl = xAl*(((1.0 - xAl)*(1.0 - xAl))*self.MAl + xCr*xAl*self.MCr + xAl*self.xFe*self.MFe)
        self.MCrAl = xCr*xAl*(self.xFe*self.MFe - (1.0 - xCr)*self.MCr - (1.0 - xAl)*self.MAl)
        self.d2fdCr2 = (-2.0*self.L0_FeCr + R*T*(1.0/(1.0 - xCr - xAl) + 1.0/xCr))/(R*T)
        self.d2fdCrdAl = (-self.L0_FeCr + self.L0_CrAl - self.L0_FeAl + R*T/(1.0 - xCr - xAl))/(R*T)
        self.d2fdAl2 = (-2.0*self.L0_FeAl + R*T*(1.0/(1.0 - xCr - xAl) + 1.0/xAl))/(R*T)
        self.G = N*K*ell*ell/self.DAl
        
    def multi_graph(self, Cr0, Al0, T, N, K, r0, ell):
        self.Cr0, self.Al0, self.T, self.N, self.K, self.r0, self.ell = Cr0, Al0, T, N, K, r0, ell
        self.GFe = 1225.7 + 124.134*T - 23.5143*T*math.log(T) - 0.439752E-2*T*T - 0.589269E-7*T*T*T + 77358.5/T
        self.GCr = - 8856.94 + 157.48*T - 26.908*T*math.log(T) + 0.189435E-2*T*T - 0.147721E-5*T*T*T + 139250.0/T
        self.GAl = - 1193.24 + 218.235446*T - 38.5844296*T*math.log(T) + 0.18531982E-1*T*T - 0.576227E-5*T*T*T + 74092.0/T
    ...
    def variables(self, Size, TotSize):
        ...
        self.fft_plan_Cr = pyfftw.FFTW(self.Cr_r, self.Cr_k, direction='FFTW_FORWARD', flags=('FFTW_MEASURE',))
        self.Al_r = np.zeros(TotSize, dtype=complex)
        self.Al_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_Al = pyfftw.FFTW(self.Al_r, self.Al_k, direction='FFTW_FORWARD', flags=('FFTW_MEASURE',))
        self.dGdCr_r = np.zeros(TotSize, dtype=complex)
        self.dGdCr_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_dGdCr = pyfftw.FFTW(self.dGdCr_r, self.dGdCr_k, direction='FFTW_FORWARD', flags=('FFTW_MEASURE',))
        self.dGdAl_r = np.zeros(TotSize, dtype=complex)
        self.dGdAl_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_dGdAl = pyfftw.FFTW(self.dGdAl_r, self.dGdAl_k, direction='FFTW_FORWARD', flags=('FFTW_MEASURE',))
        self.f11Cr_r = np.zeros(TotSize, dtype=complex)
        self.f11Cr_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_f11Cr = pyfftw.FFTW(self.f11Cr_k, self.f11Cr_r, direction='FFTW_BACKWARD', flags=('FFTW_MEASURE',))
        self.f12Cr_r = np.zeros(TotSize, dtype=complex)
        self.f12Cr_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_f12Cr = pyfftw.FFTW(self.f12Cr_k, self.f12Cr_r, direction='FFTW_BACKWARD', flags=('FFTW_MEASURE',))
        self.f11Al_r = np.zeros(TotSize, dtype=complex)
        self.f11Al_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_f11Al = pyfftw.FFTW(self.f11Al_k, self.f11Al_r, direction='FFTW_BACKWARD', flags=('FFTW_MEASURE',))
        self.f12Al_r = np.zeros(TotSize, dtype=complex)
        self.f12Al_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_f12Al = pyfftw.FFTW(self.f12Al_k, self.f12Al_r, direction='FFTW_BACKWARD', flags=('FFTW_MEASURE',))
        self.f2Cr_r = np.zeros(TotSize, dtype=complex)
        self.f2Cr_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_f2Cr = pyfftw.FFTW(self.f2Cr_r, self.f2Cr_k, direction='FFTW_FORWARD', flags=('FFTW_MEASURE',))
        self.f2Al_r = np.zeros(TotSize, dtype=complex)
        self.f2Al_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_f2Al = pyfftw.FFTW(self.f2Al_r, self.f2Al_k, direction='FFTW_FORWARD', flags=('FFTW_MEASURE',))
        self.out_fCr_r = np.zeros(TotSize, dtype=complex)
        self.out_fCr_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_outCr = pyfftw.FFTW(self.out_fCr_k, self.out_fCr_r, direction='FFTW_BACKWARD', flags=('FFTW_MEASURE',))
        self.out_fAl_r = np.zeros(TotSize, dtype=complex)
        self.out_fAl_k = np.zeros(TotSize, dtype=complex)
        self.fft_plan_outAl = pyfftw.FFTW(self.out_fAl_k, self.out_fAl_r, direction='FFTW_BACKWARD', flags=('FFTW_MEASURE',))
...
def calcPFT(Fe, Cr, Al, pars, vars, steps):
    for _ in range(steps):
        ...
        vars.dGdCr_r.real = dG0FeCrAl_dCr(
            Fe, Cr, Al, pars.GFe, pars.GCr, pars.GAl, 
            pars.L0FeCr, pars.L0FeAl, pars.L0CrAl, pars.T
        )/(R*pars.T)
        ...
        vars.f11Cr_k.real = -np.sqrt(vars.k2) * (vars.dGdCr_k.imag + pars.kappa * vars.k2 * (2.0 * vars.Cr_k.imag + vars.Al_k.imag))
        vars.f11Cr_k.imag =  np.sqrt(vars.k2) * (vars.dGdCr_k.real + pars.kappa * vars.k2 * (2.0 * vars.Cr_k.real + vars.Al_k.real))
        vars.f12Cr_k.real = -np.sqrt(vars.k2) * (vars.dGdAl_k.imag + pars.kappa * vars.k2 * (2.0 * vars.Al_k.imag + vars.Cr_k.imag))
        vars.f12Cr_k.imag =  np.sqrt(vars.k2) * (vars.dGdAl_k.real + pars.kappa * vars.k2 * (2.0 * vars.Al_k.real + vars.Cr_k.real))
        vars.f11Al_k.real = -np.sqrt(vars.k2) * (vars.dGdAl_k.imag + pars.kappa * vars.k2 * (2.0 * vars.Al_k.imag + vars.Cr_k.imag))
        vars.f11Al_k.imag =  np.sqrt(vars.k2) * (vars.dGdAl_k.real + pars.kappa * vars.k2 * (2.0 * vars.Al_k.real + vars.Cr_k.real))
        vars.f12Al_k.real = -np.sqrt(vars.k2) * (vars.dGdCr_k.imag + pars.kappa * vars.k2 * (2.0 * vars.Cr_k.imag + vars.Al_k.imag))
        vars.f12Al_k.imag =  np.sqrt(vars.k2) * (vars.dGdCr_k.real + pars.kappa * vars.k2 * (2.0 * vars.Cr_k.real + vars.Al_k.real))
        vars.fft_plan_f11Cr()
        vars.fft_plan_f12Cr()
        vars.fft_plan_f11Al()
        vars.fft_plan_f12Al()
        ...
        vars.fft_plan_f2Cr()
        vars.fft_plan_f2Al()
        vars.out_fCr_k.real = -np.sqrt(vars.k2) * vars.f2Cr_k.imag - ballistic * vars.Cr_k.real
        vars.out_fCr_k.imag =  np.sqrt(vars.k2) * vars.f2Cr_k.real - ballistic * vars.Cr_k.imag
        vars.out_fAl_k.real = -np.sqrt(vars.k2) * vars.f2Al_k.imag - ballistic * vars.Al_k.real
        vars.out_fAl_k.imag =  np.sqrt(vars.k2) * vars.f2Al_k.real - ballistic * vars.Al_k.imag
        vars.fft_plan_outCr()
        vars.fft_plan_outAl()
        if pars.Cr0 > ERROR:
