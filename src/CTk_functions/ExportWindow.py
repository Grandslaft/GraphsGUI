...
class ExportWindow(ctk.CTkToplevel):
    def __init__(self, master, **kwargs):
        ...
        self.label = ctk.CTkLabel(
            self,
            text="Експорт графіків та розрахованих значень",
            anchor="center",
        ...
        self.label.grid(
            row=0,
            column=0,
            padx=(gp.OUTER_PAD, gp.OUTER_PAD),
            pady=2 * gp.OUTER_PAD,
            sticky="nsew",
        )

        # select path frame
        self.select_path_frame = ctk.CTkFrame(self, corner_radius=gp.CORNER_RADIUS)
        self.select_path_frame.grid(
            row=1, column=0, padx=gp.OUTER_PAD, pady=(0, gp.OUTER_PAD), sticky="nsew"
        )
        ...
        self.path_text_box.grid(
            row=0, column=0, padx=gp.INNER_PAD, pady=gp.INNER_PAD, sticky="nsew"
        )
        self.path_text_box.insert("0.0", self.master.save_path)
        self.path_text_box.bind("<KeyRelease>", self.on_text_change)

        # button to call a window to choose directory
        self.choose_path = ctk.CTkButton(
            self.select_path_frame, text="Експортувати файли", command=self.select_path
        )
        self.choose_path.grid(
            row=0, column=1, padx=(0, gp.INNER_PAD), pady=gp.INNER_PAD, sticky="nsew"
        )

        # frame with data types available for extraction
        self.data_to_save = ctk.CTkFrame(self, corner_radius=gp.CORNER_RADIUS)
        self.data_to_save.grid(
            row=2, column=0, padx=gp.OUTER_PAD, pady=(0, gp.OUTER_PAD), sticky="nsew"
        )
        ...
        self.graph_export = ctk.CTkCheckBox(
            self.data_to_save, text="Експортувати рисунки", onvalue=True, offvalue=False
        )
        ...
        self.save_export_params.grid(
            row=3, column=0, padx=gp.OUTER_PAD, pady=(0, gp.OUTER_PAD), sticky="nsew"
        )

    # function which is triggered by text change event in the textbox
    def on_text_change(self, event=None):
        self.master.save_path = self.path_text_box.get(
            "1.0", "end-1c"
        )  # Save all the text in the path
    ...
    def select_path(self):
        ...
        file_path = filedialog.askdirectory(
            initialdir=self.master.save_path, title="Оберіть місце для збереження даних"
        )
